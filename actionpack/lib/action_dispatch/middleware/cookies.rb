  ...
  class Cookies
    ...
    class AbstractCookieJar # :nodoc:
      ...
      private
        ...
        def cookie_metadata(options, name)
          if request.use_cookies_with_metadata
        ...
        def parse(name, data, metadata); data; end
        def commit(options, name); end
    end

    class PermanentCookieJar < AbstractCookieJar # :nodoc:
      private
        def commit(options, name)
    ...
    class SignedKeyRotatingCookieJar < AbstractCookieJar # :nodoc:
      ...
      private
        def parse(name, signed_message, metadata)
          deserialize(name) do |rotate|
            @verifier.verified(signed_message, on_rotation: rotate, purpose: metadata[:purpose])
          end
        end

        def commit(options, name)
          options[:value] = @verifier.generate(serialize(options[:value]), cookie_metadata(options, name))
    ...
    class EncryptedKeyRotatingCookieJar < AbstractCookieJar # :nodoc:
      ...
      private
        def parse(name, encrypted_message, metadata)
          deserialize(name) do |rotate|
            @encryptor.decrypt_and_verify(encrypted_message, on_rotation: rotate, purpose: metadata[:purpose])
          end
        ...
        def commit(options, name)
          options[:value] = @encryptor.encrypt_and_sign(serialize(options[:value]), cookie_metadata(options, name))
